class Midi_Dialog {
  ...
  create_midi_dialog(win) {
    ...
    ctrl_cont.appendChild(create_btn(20, 6, 12, 'open_file', midi_dialog.open_midi_file));
    ctrl_cont.appendChild(create_label(38, 6, 'open file', 10))
    ctrl_cont.appendChild(create_label(20, 22, '- filename -', 10,'midi_filename'))
    ctrl_cont.appendChild(create_btn(20, 54, 12, 'to_editor', midi_dialog.set_selection_to_editor));
    ...
    ctrl_cont.appendChild(create_nr(20, 90, 36, 1, 1000, 1, midi_dialog.set_midi_start_bar));
    ctrl_cont.appendChild(create_label(58, 90, 'start', 10));
    ctrl_cont.appendChild(create_nr(90, 90, 36, 1, 1000, 1, midi_dialog.set_midi_end_bar));
    ctrl_cont.appendChild(create_label(128, 90, 'end', 10));
  
    var sel = create_select(158, 90, midi_dialog.select_track, 'midi_dialog.select_track')
    ctrl_cont.appendChild(sel );
    ctrl_cont.appendChild(create_btn(20,140, 12, 'midi_dialog.midi_to_player', midi_dialog.midi_to_player));
  ...
  midi_to_player(e){
    try {
      ...
      for (i = 0; i < arr.val_array.length ; i++) {
        ...
        send_new_item(el[0],el[1],el[2]+1,el[3]+1,el[4],el[5],el[6],el[7],el[8])
      }
    }
    catch(e){log(e)}
  }
